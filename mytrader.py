...
class AutoTrader(BaseAutoTrader):
    ...
    def __init__(self, loop: asyncio.AbstractEventLoop, team_name: str, secret: str):
        ...
        self.last_calc = 0
        self.last_ask_etf = 0
        self.last_ask_futures = 0
        self.prices = pd.DataFrame(data={"ts": [], "ask_f": [], "ask_e": []})
        self.prices.set_index("ts")
        self.long_market = self.short_market = False
        self.pl = 0
        self.ps = 0
    ...
    def on_order_book_update_message(
        ...
        if instrument == Instrument.FUTURE:
            self.last_ask_futures = ask_prices[0]
        else:
            self.last_ask_etf = ask_prices[0]

        self.prices.loc[ts] = [ts, self.last_ask_futures, self.last_ask_etf]
        self.prices = self.prices[self.prices["ts"] >= ts - WINDOW_SIZE]
        self.last_ts = ts

        if self.prices.size < WINDOW_SIZE:
            return

        self.last_calc = ts
        ask_e = self.prices["ask_e"].values
        ask_f = self.prices["ask_f"].values
        self.hedge_ratio = np.linalg.lstsq(
            np.vstack([self.prices["ask_e"], np.ones(len(ask_e))]).T,
            self.prices["ask_f"],
            rcond=None,
        )[0][0]

        spread = ask_f - self.hedge_ratio * ask_e
        zscore_last = ((spread - spread.mean()) / spread.std())[-1]

        if (
            zscore_last >= ZSCORE_HIGH
            and not self.long_market
            and ask_e[0] > 0
            and LOT_SIZE + self.position <= POSITION_LIMIT
        ):
            print(f"Long market {self.pl}")
            self.long_market = True
            self.bid_id = next(self.order_ids)
            self.send_insert_order(
                self.bid_id, Side.BID, ask_prices[0], LOT_SIZE, Lifespan.F
            )
            self.bids.add(self.bid_id)
            self.logger.info("long market")
        if abs(zscore_last) <= ZSCORE_LOW and self.long_market:
            print(f"Long market close {self.pl}")
            self.long_market = False
            self.send_cancel_order(self.ask_id)
            self.ask_id = 0
            self.logger.info("close long market")
        if (
            zscore_last < -ZSCORE_HIGH
            ...
            print(f"Short market {self.ps}")
            self.short_market = True
            self.ask_id = next(self.order_ids)
            self.send_insert_order(
                self.ask_id, Side.ASK, bid_prices[0], LOT_SIZE, Lifespan.F
            )
            self.asks.add(self.ask_id)
            self.logger.info("short market")
        if abs(zscore_last) <= ZSCORE_LOW and self.short_market:
            print(f"Short market close {self.ps}")
            self.short_market = False
            self.send_cancel_order(self.ask_id)
            self.ask_id = 0
            self.logger.info("close short market")
